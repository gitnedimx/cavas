...
class CreditCode(models.Model):
    ...
    credit_check = fields.Selection([('credit_hold', 'Credit Hold'),
                                    ...
                                    string='Check Credit', default='check_limit', required=True,
    ...
    def check_code_unique(self):
        for rec in self:
            if rec.code:
                find_code = self.search([('code', '=', rec.code), ('id', '!=', rec.id)])
                if find_code:
    ...
    def calculate_onorder_amount(self, partner, operator_condition, days, sale_order, partner_currency_id):
        ...
        account_invoice_search = self.env['account.move'].search([('partner_id', '=', partner), ('state', '=', 'draft')])
        if account_invoice_search:
            for account_invoice in account_invoice_search:
                if account_invoice.invoice_payment_term_id:
                    totlines = account_invoice.invoice_payment_term_id.with_context(currency_id=account_invoice.currency_id.id).compute(account_invoice.amount_total, account_invoice.invoice_date)[0]
            ...
            if credit_check == 'credit_hold':
                ...
                for account_invoice in account_invoice_search:
                    if account_invoice.invoice_payment_term_id:
                        ...
                        if account_invoice.invoice_date_due and delivery_date >= account_invoice.invoice_date_due:
                            if account_invoice.type == 'out_invoice':
...
class AccountInvoice(models.Model):
    ...
    def _onchange_payment_term_date_invoice(self):
        ...
        if not self.invoice_payment_term_id:
            ...
            pterm_list = pterm.with_context(currency_id=self.company_id.currency_id.id).compute(value=1, date_ref=invoice_date)[0]
